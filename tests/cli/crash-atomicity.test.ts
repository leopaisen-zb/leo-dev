...
describe('controller crash-atomic batches', () => {
  ...
  test('recovers review projection after a crash without exposing done/active-lease or manifest/journal halves', async () => {
    ...
    const path = await receipt({ receiptId: 'review-crash', provenance: 'platform-attested', actorLabel: 'self review', sessionId: 'session', ...context, findingsHash: digest('none'), verdict: 'pass', timestamp: issuedAt(), expiresAt: expiresAt() });
