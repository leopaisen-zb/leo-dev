...
5. 新变更先运行 `leo-dev init --change <id> --spec <path>`，再 `leo-dev route`，再进入 discovery。目标和范围在本会话对齐且用户说「开工」后，运行 `leo-dev start --change <id> --goal <text>`，然后 spec-review → spec-approved（开工授权）。不必等用户再批规格文件。规格在 `init --spec` 绑定；不要在 start 后再写规格/计划，也不要再 `route`。目标变了才停下来问。ask exactly one highest-leverage material question at a time。
