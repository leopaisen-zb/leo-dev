...
开工可执行顺序：用户说开工后先写规格和计划，再 `init --spec` 与一次 `leo-dev route`，进入 discovery，再运行 `leo-dev start --change <id> --goal <text>`，然后 spec-review → spec-approved。不必等用户再批规格文件。不要二次 `route`。目标仓库没有 `core/gates/default.yaml` 时，不要抄别的仓库的 `npm run typecheck`；用 `--registry` 指向这个仓库里已有命令的 registry，没有真实命令就停下来问人。独立审查通过且证据绑当前树之后运行 `leo-dev commit --change <id> --message <text>`；不准 push。
