...
3. Text search shall be case-insensitive and match either task titles or task notes. Empty queries match all tasks. Priority and text filters combine. Clearing filters restores all tasks; no matches shows an explicit empty state. Task counts describe visible cards.
...
Acceptance tests are owned by a separate author; implementers may read them but not edit them. Version 2 extends search to notes. It is a coordinator-authored refinement under the user's delegated example design, recorded as a new specification revision with fresh evidence for every task; it is not a fabricated new direct human decision.
